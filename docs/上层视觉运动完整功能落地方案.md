...
> D06 已收敛本期目标为草莓 demo：本文原有蓝莓实现、双果品模型/参数和双果品验收均不作为本期交付要求；以正在编制的六阶段技术文档为最终实施规格。蓝莓口令的非执行行为等待 D07。
...
交付一个同进程、同步阻塞的分拣程序：冷启动检查并恢复 home → 等待语音 → ASR → LLM 选择草莓/蓝莓方案 → 每轮重新采集视觉 → plans 分级和分配落点 → 运动执行 → 分发结果 → 完成后重新等待下一条语音。
...
以上是静态核对结果，未运行真机或宣称现有测试通过。公共接口的首个交付应是“类型、调用方、校验、测试、文档一起可用”。
...
1. 注册无运动副作用的终止日志与资源清理；加载 app、prompt、云适配器配置、运动 profile，校验两个方案映射。
2. 完成纯资源检查及 VAD 初始化；构造 motor/arm。连接、设备身份核验和显式力矩使能必须区分，不能把 `Goal_Position` 写入视为无害预置。
3. 接入现有驱动时显式使用初始化参数，先只读核验当前反馈与设备状态，再进入已验证的使能流程；初始化异常清理由驱动承担，shutdown 不补发卸力指令。CAL-052 的已知行为需纳入启动测试。
4. 按每关节 `motion.settle_position_tol_deg` 判断 home：容差外执行 `open_gripper()` → 依次经过已验证的安全中转路线 → `move_joints(home)`；容差内不搬臂。
5. 校验静止条件，并在恢复后再次确认 home。多中转点时冻结回程顺序，复用运动 RETURN 的约定；当前只有一个点不能掩盖未来顺序问题。
6. 调用 `vision.configure()`、`vision.init()`，一次性加载权重、启动相机并验证可采集。init 阶段故障不能伪装成 NoTarget。
7. 打印就绪状态，进入 WAITING。

启动接入还需补齐驱动生命周期：当前 `initialize()` 直接进入逐轴力矩配置，不能据此认为“当前位置与旧 Goal_Position 已核对”或“部分使能失败已有完整清理”。将已有标定脚本验证过的初始化步骤收敛到驱动公开接口，并测试旧目标不一致、部分使能失败及反馈中断。main 不直接操作驱动私有寄存器或修改 `_torque_enabled`；驱动初始化阶段的失败清理与运行期 terminate 的零新指令规则分别测试。
...
| 其余尺寸 | **建议补充**：内部点云同时验证长、宽、高不超完整运动包络。原文只写闭合轴上限，不能据此推导整个三维包络成立；若不扩展过滤，则必须由已测演示样本保证其它两轴上界 |
...
while True:
    ...
    if not exhausted and self.consecutive_failures >= MAX_CONSECUTIVE_FAILURES:
        terminate_result(result)  # 不做人工确认、解锁或回零

    self.recover_if_required(result)
    if exhausted:
        log_completion("IGNORE_EXHAUSTED")
        return
```

> **D14 修订（2026-09-22）**：上文骨架中 `exhausted` 分支（恢复后打印 `IGNORE_EXHAUSTED` 完成）已废止——封顶改为 `terminate("NO_GRASPABLE_TARGET")`，不恢复、不打印"任务完成"；以 P1 §6.2 为准。
...
人工确认建议要求明确输入 `已空爪`；空回车不算确认，EOF/Ctrl-C 进入 terminate。确认期间不后台请求视觉或发送指令；操作者若不能确认，不进入 reset。`reset_fault()` 的当前实现不能证明物体已被移走，也不提供完整静止 dwell 证明，恢复助手须核验静止条件，不能把 reset 成功当成全部现场条件已满足。

回程助手只调用运动公开原语，依次经过已校验的安全中转点再到 home，并确认到位。正常 SUCCESS 已由 RETURN 保证 home，不额外重复回零。封顶要求恢复时，必须恢复成功才打印完成；不能在最后一次失败后直接返回 WAITING。
...
| 任务结束 | NO_TARGET_STREAK / IGNORE_EXHAUSTED、成功数、各失败码计数、已占用槽位数 |
| 终止 | 所处阶段、原异常、最后一次已知运动状态、是否无法保持 |

建议终端输出示例：`任务完成：结束原因=IGNORE_EXHAUSTED，成功=2，抓空=1，已分配优品槽位=3`。保留文档要求的“任务完成”，同时避免把未抓走目标掩盖成成功。
...
| P5 目标机全链联调 | 实物从单目标到多目标；草莓/蓝莓分别验收；故障与连续任务测试 | 固定版本、依赖锁定、现场操作说明、测试报告、回放日志及可重复 demo |
...
```bash
python3 main.py --mode mock --config configs/app.mock.json
python3 main.py --mode real --config configs/app.local.json
```

mock 必须完全不打开真实串口，使用 simulation profile 与确定的离线数据源；真实设备和云服务分别在模块联调脚本中验证。生产 real 的任务完成始终回 WAITING，不增加默认“跑完一次就退出”的分支。

启动前确认：空爪、冷启动恢复通道已清、相机固定、ROI 内单层且只有一种果品、A 槽位已清空、箱内余量充足、语音任务与果品一致。任务结束后程序继续等待，下一条任务发出前清空/更换放置区；因为新实例会从 A_01 重新分配，不能连续往未清空托盘复用槽位。
