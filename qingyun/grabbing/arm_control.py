...
class ArmController:
    ...
    def _step_gripper_open(self, target_pct: float, stage: str, timeout_s: float) -> float:
        ...
        # open_speed_pct_s 是目标开度的命令变化率，不是反馈速度或瞬时物理速度保证。
        # CAL-082 demo空爪20%/s实测约18.50%/s且有动态滞后；必须继续检查反馈与到位，
        # 不得仅以“开度差 / 命令速度”作为动作完成，也不据此宣称带物释放安全。
        # CAL-092三次空爪到55%（含0.15s驻留）实测3.4668~3.5344s，
        # 三轮无异常确认，demo超时已写4.3s；非全100%/带物时限证明。
        # CAL-096/097复用三轮55%空爪到位窗口（各6帧、跨度≥0.15s），
        # max误差0.3505%/反馈速度0加3个反馈计数余量向上取整，demo
        # 到位阈值0.6%/0.3%/s；重放首次完成时间不变、仍在4.3s内。
        # 18帧不是18次独立循环，零速度反馈非物理静止证明，不外推带物/全行程。
        gp = self.params.gripper
    ...
    def _close_until_contact(self) -> HoldingState:
        ...
        """
        # CAL-083 的15%/s仅在空爪75%→25%验证（实测约14.50%/s），不是果实安全速度。
        # CAL-084~088接触与延迟/额外压缩预算完成前不得据空爪通过就执行真实完整抓取；
        # 接触试验若要求降低close_speed_pct_s，必须重验本阶段及其空闭合/超时下游。
        # CAL-085增强塑料假果试验的闭合跟踪25raw/候选停止39mA仅用于有界采集，
        # 不得移植为运行期通用容差或果实压力安全值；1801raw同目标空爪短保持
        # 重放10mA零误报，候选仍待空爪异常/增强轮实体夹紧稳定确认，且不证明
        # _confirm_contact改持反馈开度后的持力；不得绕过draft/verification门。
        # CAL-086按现有fitter得条件候选20mA（10×2），非损坏边界；必须与
        # contact10配套，不能单写到当前contact220配置。正常数据零超限
        # 不证明超限停止响应，故两候选未集成，仍禁止据此执行真实完整抓取。
        # CAL-087离线比较1/2/3/4/5/7帧后条件保留3：候选10mA、dwell0.1s
        # 下记录空爪零误报；0.1334s是电流信号到重放命中的时间，非测得
        # 物理接触/压缩延迟，亦未验证15%/s与反馈开度保持后的持力。
        # CAL-088固定候选10mA/3帧比较后条件保留0.1s；采样抖动可使
        # 2/30s与0.1s同一帧才命中，不得将秒级连续确认换成固定帧数。
        # CAL-084~088证据整理不等于小组已验收，仍未集成两电流候选。
        # CAL-085反馈保持补充已仅完成空爪张55%后卸力准备（反馈2264raw），
        # 张开无异常与新假果20mm带摆放已确认，但不是轻触/夹紧确认；
        # 尚无新反馈保持持物证据，不据此验收CAL-093或自动夹紧。
        # 新名义轻触接近仅6号1%/s至1866、反馈1870即卸力，用户确认仍有间隙；
        # 不是首次轻触/压缩零点，后续小步接近须重新批准，不从13mA推已接触。
        # 已另获批准仅6号小步至1852、反馈1858即卸力，峰6.5mA；用户确认
        # 刚轻触且无明显压缩，1858仅本次摆放操作参考，不是精确初触/压缩零点。
        # 无末端保持，尚非反馈开度保持的稳定持物证据；候选10mA未集成。
        # 新候选→反馈保持首轮在命令1844/反馈1855、跟踪11raw时按10raw门
        # 停止，峰13mA；未形成0.1s接触确认、未进入保持，已卸力恢复。
        # 受阻位置误差不能等同碰撞/故障，也不能自动放宽门或继续压夹；
        # 待现场观察与另行有界方案，不从失败样本验收085/093。
        # 用户报仍轻触/可能手动移位并要求新试验；保留1837绝对下限和
        # 所有保护，仅重试一次，反馈1844/峰6.5mA未命中候选，未保持即卸力。
        # 用户报末端仍轻触；末帧有运动速度反馈，不推已驻留/夹紧，禁止自动补夹。
        # 已另批准保留1837下限同命令确认：实际0.1686s/6帧、反馈1842，
        # 峰6.5mA、候选均值峰4.333mA，未命中且未切反馈保持，已卸力恢复。
        # 命令确认驻留不是反馈保持/稳定夹紧证据，待现场观察，不自动降阈值。
        # 本轮闭合16帧反馈全1844，确认阶段才到1842，存在2raw实际移动；
        # 每tick名义0.466raw量化重复目标，不能断言完全未动或确认为死区。
        # 1830已另批准仅1轮：实际1842→1839，确认0.1685s/6帧，峰13mA但
        # 3帧均值峰8.667mA未合格，未持反馈，已卸力恢复；用户明确已夹紧。
        # 物理夹紧与候选未识别均保留，不能从该回复验收反馈保持/滑移/承重；
        # 已另批准完成实际反馈1839固定目标9帧/0.2685s，全位置1839、电流0；
        # 已卸力恢复，待实体仍夹紧/轻触/松脱观察，不从位置稳定验收持物。
        # 这是人确认初态的独立标定原语，不修改/绕过运行期接触阈值和draft门。
        # 用户确认1~5主动调整，单轴标定不以臂姿态/域阻断；但保留越界
        # 诊断，不放宽运行期臂限位，也不从该接近数据推反馈保持持物成立。
        # CAL-089/090三轮低速空闭合中位数均1502raw≈0.2146%，候选empty
        # 0.21%+tol1%已获三轮无异常确认并写入demo；混合主动改变的2号支撑姿态，
        # 不从27帧burst推27次独立重复/真实方差为0，亦不作15%/s验收。
        # CAL-091三次空爪55%闭至反馈≤1.21%实测3.9019~3.9036s，三轮
        # 无异常确认，demo超时已写4.7s；诊断20mA/120及80raw保护
        # 不移植为生产安全认证。非全开度/带物最坏时限，不追加压紧保持。
        # 2026-09-18状态更新（上方旧1%/s候选批注仅存档）：CAL-085~088已按
        # 生产15%/s重测写回contact13/hard/window3/dwell0.1；稳定持物round2
        # 实机发现硬摆位闭合电流4帧冲104mA、本循环monitor先于0.1s确认停机
        # 的竞态，操作者选方案d将CAL-086上调150mA消除（策略决定，非损坏
        # 上界证据；monitor仍保留hard停机语义）。真实完整抓取仍待CAL-093~
        # 095/098与draft/verification门，禁止据本批注绕过。
        gp = self.params.gripper
        q_hold = self._held_joint_target()
        cmd = float(self._current_gripper_pct())
        ...
        while True:
            ...
            # CAL-084允许小/大目标在不同深度接触，gap始终是同一gap表参考平面坐标。
            # 它只提供开度门控，不是其他接触深度的实际间距、压缩量或桌面净距证明。
