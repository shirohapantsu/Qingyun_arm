#!/usr/bin/env python3
"""CAL-058：当前姿态恢复到中转位后，连续完成五轴速度阶梯并卸力。"""
...
from scripts.calibrate import new_run_id, new_sample_id  # noqa: E402
...
OFFSETS_DEG = np.array([30.0, 30.0, 30.0, 20.0, 30.0])
SPEED_LEVELS_DEG_S = (
    np.array([20.0] * 5),
    np.array([40.0] * 5),
    np.array([60.0, 60.0, 55.0, 60.0, 60.0]),
)


class CaptureExecutor(SyncExecutor):
    """复用生产执行器，并把每次真实反馈保存为 motion payload。"""
    ...
    def read_feedback(self):
        ...
        if self.context is not None:
            ...
            self.context_rows.append({
                ...
                "requested_peak_speed_deg_s": float(
                    self.context["requested_peak_speed_deg_s"]
                ),
            })
        return feedback

    def play_captured(self, segment, *, axis_index: int, requested_speed: float) -> None:
        delta = float(segment.end_joints[axis_index] - segment.start_joints[axis_index])
        if abs(delta) <= 1e-9:
            raise ValueError("CAL-058 采集段不能是零位移")
        self.context = {
            "trajectory_id": segment.name,
            "segment": segment.name,
            "direction": 1 if delta > 0.0 else -1,
            "active_axis": JOINT_NAMES[axis_index],
            "requested_peak_speed_deg_s": requested_speed,
        }
        self.context_rows = []
        try:
...
def _candidate_params(params, axis_index: int, speed_deg_s: float):
    velocity = np.asarray(params.motion.max_velocity_deg_s, float).copy()
    step = np.asarray(params.motion.max_command_step_deg, float).copy()
    velocity[axis_index] = speed_deg_s
    step[axis_index] = min(step[axis_index], speed_deg_s / params.timing.fps)
    return replace(
        params,
        motion=replace(
            params.motion,
            max_velocity_deg_s=velocity,
            max_command_step_deg=step,
        ),
    )
...
def _build_test_segments(params, waypoint: np.ndarray, gripper_pct: float):
    ...
    segments = []
    preflight = []
    for level_index, speeds in enumerate(SPEED_LEVELS_DEG_S, start=1):
        side = 1.0 if level_index % 2 else -1.0
        for axis_index, requested_speed in enumerate(speeds):
            target = waypoint.copy()
            target[axis_index] += side * OFFSETS_DEG[axis_index]
            planning_params = _candidate_params(params, axis_index, float(requested_speed))
            for leg_index, (start, end) in enumerate(
                ((waypoint, target), (target, waypoint)), start=1
            ):
                name = f"cal058_L{level_index}_J{axis_index + 1}_leg{leg_index}"
                ...
                segments.append((segment, axis_index, float(requested_speed)))
                preflight.append({
                    "trajectory_id": name,
                    "active_axis": JOINT_NAMES[axis_index],
                    ...
                    "requested_peak_speed_deg_s": float(requested_speed),
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="从1开始的速度扫描段编号，用于安全中断后的续采",
    )
    parser.add_argument("--settle-position-tolerance-deg", type=float, default=2.6)
    ...
    if args.raw_output.exists() or args.report_output.exists():
        raise SystemExit("拒绝覆盖既有 CAL-058 证据")
    if args.recovery_duration_s < 10.0:
        raise SystemExit("恢复段至少 10 秒")
    ...
    # 恢复例外只跳过当前姿态到中转位的碰撞判定；限位、步长、关节/TCP动态
    # 仍由生产 validate_segment 校验。速度扫描在下方重新使用完整 checker。
    ...
    report: dict[str, Any] = {
        "schema_version": 1,
        "parameter_id": "CAL-058",
        "test": "current_to_waypoint_then_joint_velocity_sweep_then_torque_off",
    ...
    try:
        ...
        for index, (segment, axis_index, requested_speed) in enumerate(
            selected_test_segments, start=args.start_segment_index
        ):
            print(
                f"CAL-058 {index}/{len(test_segments)} {segment.name} "
                f"target={requested_speed:.0f}deg/s",
                flush=True,
            )
        ...
        speed = np.abs(np.asarray([row["speed_deg_s"] for row in executor.rows], float))
        current = np.abs(np.asarray([row["current_ma"] for row in executor.rows], float))
        peak_speed = np.max(speed, axis=0)
        peak_current = np.max(current, axis=0)
        report["sample_rows"] = len(executor.rows)
        report["measured_peak_velocity_deg_s"] = [float(v) for v in peak_speed]
        requested_max = np.max(np.asarray(SPEED_LEVELS_DEG_S, float), axis=0)
        if np.any(peak_speed > requested_max * 1.5):
            raise RuntimeError(
                "速度反馈峰值与命令量级不一致，拒绝生成候选：feedback="
                + json.dumps([float(v) for v in peak_speed])
                + "; requested="
                + json.dumps([float(v) for v in requested_max])
            )
        report["proposed_max_velocity_deg_s"] = [
            round(float(v) * 0.85, 2) for v in peak_speed
        ]
    ...
    if failure is not None:
        raise SystemExit(f"CAL-058 失败：{failure}; evidence={args.report_output}")
    if report.get("torque_enabled_after") != [0] * 6:
        raise SystemExit("CAL-058 运动完成，但未确认六轴全部卸力")
    print(
        "CAL-058 采集通过并已卸力；候选 max_velocity="
        + json.dumps(report["proposed_max_velocity_deg_s"])
